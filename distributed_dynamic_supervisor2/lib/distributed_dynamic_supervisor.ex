defmodule DistributedDynamicSupervisor do
  ...
  @compile {:inline, register_name: 2, partition_sup: 1, via_partition_sup: 2}

  ## API

  @doc """
  Returns the child spec for the global supervisor.
  """
  @spec child_spec(atom()) :: {module(), keyword()}
  def child_spec(name \\ __MODULE__) do
  ...
  """
  @spec start_child(atom(), child_spec(), any()) :: {:ok, pid()} | {:error, term()}
  ...
  def terminate_child(name \\ __MODULE__, key) do
    case lookup(name, key) do
      nil -> {:error, :not_found}
      pid -> DynamicSupervisor.terminate_child(via_partition_sup(name, key), pid)
    end
  end
  ...
  """
  @spec register_name(atom(), any()) :: {:via, :syn, {module(), any()}}
